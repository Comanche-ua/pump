...
Состав картинки (три панели, тёмная тема):
  ...
  • RSI 14 с уровнями 30/50/70 + маркеры и заливка зон;
  • стохастик (14, 3): %K и %D с уровнями 20/80 + маркеры и заливка зон.
Числа НЕ рисуются на картинке: текст отдаётся подписью в Telegram
(`chart_caption`). Это избавляет от растрового шрифта и даёт выделяемые значения.
"""
...
RSI_LINE = (240, 185, 11)
LEVEL = (58, 66, 86)
LEVEL_STRONG = (78, 90, 115)          # более заметные 30/70 и 20/80
STOCH_K = (0, 188, 212)
STOCH_D = (233, 30, 99)
LAST_LINE = (120, 132, 156)

# Заливка зон (очень лёгкая)
ZONE_OVER = (45, 30, 30)              # перекупленность
ZONE_UNDER = (25, 40, 35)             # перепроданность
...
class Canvas:
    ...
    def line(self, x1: int, y1: int, x2: int, y2: int, color, thickness: int = 1) -> None:
        """Отрезок по Брезенхэму. thickness=2 делает линию чуть толще."""
        def plot(x, y):
            self.px(x, y, color)
            if thickness >= 2:
                self.px(x + 1, y, color)
    ...
    def circle(self, cx: int, cy: int, r: int, color, fill: bool = True) -> None:
        """Маленький круг (маркер)."""
        for dy in range(-r, r + 1):
            for dx in range(-r, r + 1):
                if dx * dx + dy * dy <= r * r:
                    if fill or abs(dx * dx + dy * dy - r * r) < r:
                        self.px(cx + dx, cy + dy, color)

    def cross(self, cx: int, cy: int, size: int, color) -> None:
        """Крестик (для пересечения %K/%D)."""
        for i in range(-size, size + 1):
            self.px(cx + i, cy + i, color)
            self.px(cx + i, cy - i, color)
...
# ── Отрисовка ──
def _panel_scale(top: int, height: int, lo: float, hi: float):
    span = (hi - lo) or 1.0
...
def _fill_zone(c: Canvas, x0: int, x1: int, to_y, y_lo: float, y_hi: float, color):
    """Заливает горизонтальную зону между двумя уровнями."""
    ya = to_y(y_hi)
    yb = to_y(y_lo)
    top, bot = min(ya, yb), max(ya, yb)
    for y in range(top, bot + 1):
        c.hline(x0, x1, y, color)
...
def render_chart(symbol: str, candles, bars: int = 120,
    ...
    to_y = _panel_scale(p_top, PRICE_H, lo_p, hi_p)
    ...
    to_r = _panel_scale(r_top, RSI_H, 0.0, 100.0)

    # Заливка зон
    c.set_clip(x0, r_top + 1, x1, r_bot - 1)
    _fill_zone(c, x0, x1, to_r, 70, 100, ZONE_OVER)
    _fill_zone(c, x0, x1, to_r, 0, 30, ZONE_UNDER)

    # Уровни
    for lvl, col in ((30, LEVEL_STRONG), (50, GRID), (70, LEVEL_STRONG)):
    ...
    for i, v in enumerate(rsi):
        ...
        if prev is not None:
            c.line(prev[0], prev[1], x, y, RSI_LINE, thickness=2)
        prev = (x, y)
        last_rsi_xy = (x, y)
    ...
    if last_rsi_xy:
        lx, ly = last_rsi_xy
        c.circle(lx, ly, 4, (255, 255, 255), fill=True)   # обводка
        c.circle(lx, ly, 3, RSI_LINE, fill=True)
    ...
    to_s = _panel_scale(s_top, STOCH_H, 0.0, 100.0)

    c.set_clip(x0, s_top + 1, x1, s_bot - 1)

    # Заливка зон
    _fill_zone(c, x0, x1, to_s, 80, 100, ZONE_OVER)
    _fill_zone(c, x0, x1, to_s, 0, 20, ZONE_UNDER)

    # Уровни
    for lvl, col in ((20, LEVEL_STRONG), (50, GRID), (80, LEVEL_STRONG)):
        c.hline(x0, x1, to_s(lvl), col, dashed=True)

    # Линии %K и %D
    last_k_xy = last_d_xy = None
    for series, color in ((k, STOCH_K), (d, STOCH_D)):
        ...
        for i, v in enumerate(series):
            ...
            if prev is not None:
                c.line(prev[0], prev[1], x, y, color, thickness=2)
            prev = (x, y)
            if series is k:
                last_k_xy = (x, y)
            else:
    ...
    # Маркеры текущего %K и %D
    if last_k_xy:
        lx, ly = last_k_xy
        c.circle(lx, ly, 4, (255, 255, 255), fill=True)
        c.circle(lx, ly, 3, STOCH_K, fill=True)
    if last_d_xy:
        lx, ly = last_d_xy
        c.circle(lx, ly, 4, (255, 255, 255), fill=True)
        c.circle(lx, ly, 3, STOCH_D, fill=True)

    # Крестик при пересечении на последнем баре (если было)
    if (len(k) >= 2 and k[-1] is not None and d[-1] is not None
            and k[-2] is not None and d[-2] is not None):
        cross_up = k[-2] < d[-2] and k[-1] > d[-1]
        cross_dn = k[-2] > d[-2] and k[-1] < d[-1]
        if cross_up or cross_dn:
            # ставим крестик примерно посередине между K и D
            mx = last_k_xy[0] if last_k_xy else last_d_xy[0]
            my = (last_k_xy[1] + last_d_xy[1]) // 2 if (last_k_xy and last_d_xy) else last_k_xy[1]
            c.cross(mx, my, 5, (255, 255, 180))
...
def chart_caption(symbol: str, candles, bars: int = 120) -> str:
    ...
    lines = [
        ...
        "",
        "<i>Верх: свечи + Боллинджер · Середина: RSI (зоны 30/70) · "
        "Низ: стохастик (зоны 20/80) — %K бирюзовый, %D розовый. "
        "Точки = текущие значения, крестик = пересечение %K/%D.</i>",
    ]
    return "\n".join(lines)
